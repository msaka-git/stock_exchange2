...
class hisse():
    ...
    def fiyat_sorgula(self):
        ...
        for tag in soup.find_all('strong', {'class': 'mr-4'}):
            #print(tag) # butun hisseleri listeliyor
            for anchor in tag.find_all('a'):
                links.append(anchor['href'])
                #print(links) #butun hisseleri liste formatinda yaziyor



        for h in links:
            url_fiyat = "http://finans.mynet.com" + h
            #print(url_fiyat) #butun hisselerin linkini veriyor

            self.url_fiyat_gwrate=url_fiyat + "bilanco" + "/" + str(year1) + "-12" + "/" + "1"


            self.url_fiyat_gwrate2=url_fiyat + "bilanco" + "/" + str(year2) + "-12" + "/" + "1"


            #self.url_fiyat_info=url_fiyat + "sirket-bilgileri" # ARTIK CALISMIYOR
            self.url_fiyat_info="https://yatirim.akbank.com/tr-tr/hisse-senedi/Sayfalar/hisse-senet-detay.aspx?hisse={}".format(self.a.upper())
            ...
            if self.a in url_fiyat:
                ...
                hisse_fiyati=soup_fiyat.find_all("li",{"class":"flex align-items-center justify-content-between"})

                an=hisse_fiyati[0].find_all('span')
                myit = iter(an)
    ...
    def peg_calcul(self):
        # Hisse ucuz degerlendirgine hukmedebilmek icin peg 1'den kucuk olmali.
    ...
    def base_growth_rate(self):
        ...
        try:
            ...
            #hisse_fiyati1 = soup_fiyat1.find_all("td", {"data-title-value": "Net Dönem Karı/Zararı_1"}) # Eski kod calismiyor
            hisse_fiyati1=soup_fiyat1.select_one('strong:contains("Net Dönem Karı/Zararı") + span').text

            #hisse_fiyati1 = hisse_fiyati1[0] # Eski koda bagli. calismiyor
            my_string1 = hisse_fiyati1


            commas_removed1 = my_string1.replace(',', '.')  # remove comma separation
            commas_removed1 = commas_removed1.replace('.','')
            commas_removed1 = commas_removed1[:-2]
            ...
            hisse_fiyati2=soup_fiyat2.select_one('strong:contains("Net Dönem Karı/Zararı") + span').text
            my_string2 = hisse_fiyati2

            commas_removed2 = my_string2.replace(',', '.')  # remove comma separation
            commas_removed2 = commas_removed2.replace('.', '')
            commas_removed2 = commas_removed2[:-2]
        ...
        try:
            if (self.gw > float(0)) and (self.gw <= float(30)):
                if (self.fv < self.my_float) and (self.peg_yearly < float(1)) and (self.my_float_pd_dd < float(1)) and \
                    ...
                    return "PEG ve pd/dd olması gerekenin altında.\nYıllık bazda hisse makul gibi gözüküyor. Fair Value'ye (Adil fiyat) bak.\n" \
...
#Hisse.pd_dd()
#Hisse.peg_calcul()
#Hisse.base_growth_rate()
#Hisse.base_growth_rate5y()
#Hisse.base_growth_rate10y()
#Hisse.peg_yearly()